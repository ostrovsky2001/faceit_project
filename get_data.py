...
next_matches = ['1-b94d0662-d21b-4308-8576-ac698b9d30a8']
print(next_matches)
...
for match_id in next_matches:
	...
	if players_id !=701:
		...
		prev_gather_matchs = asyncio.run(get_info(players = players_id,timestamp_to=timestamp_to,was_parsing = was_parsing,next_matches = next_matches))
		for gather_match in prev_gather_matchs[1]:
			if gather_match[0]!=702:
				stats_previos_match = asyncio.run(get_stats(last_matches = gather_match[0], player = gather_match[1]))
				for i in stats_previos_match:
					if i ==704:
						stats_previos_match.remove(i)
		...
		# 	if last_matches!=702:
		# 		stats_previos_match = asyncio.run(get_stats(last_matches, player))
		# 		for i in stats_previos_match:
		# 			if i ==704:
		# 				stats_previos_match.remove(i)
